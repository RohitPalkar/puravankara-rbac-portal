...
import type { TableColumn, LeafColumn, SortState } from '../hooks/types';
...
function hasGroups(columns: TableColumn[]): boolean {
  return columns.some((col) => isGroupedColumn(col));
}
...
export function GroupedHeaderTable({
  ...
  const grouped = hasGroups(columns);

  const renderSortIcon = (col: LeafColumn) => {
          ...
          <Table stickyHeader={stickyHeader} size="small">
            <TableHead>
              ...
              {grouped ? (
                <TableRow>
                  {columns.map((col, idx) => {
                    if (isLeafColumn(col)) return null;
                    if (isGroupedColumn(col)) {
                      return col.children.map((child, cIdx) => (
                        <TableCell
                          key={`sub-${child.field}-${idx}-${cIdx}`}
              ...
              {filteredRows.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={leafCols.length} sx={{ borderBottom: 'none' }}>
                    {emptyState || (
                      <TableEmptyState title="No data found" />
                    )}
                  </TableCell>
                </TableRow>
              ) : (
