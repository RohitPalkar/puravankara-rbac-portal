...
export class SetupService {
  ...
  async reset(): Promise<void> {
    ...
    // Clear business masters only
    await q(`DELETE FROM channel_partners`);
    await q(`DELETE FROM phases`);
    ...
    this.logger.log(
      'Reset complete: non-admin users removed; business masters cleared',
    );
  }
}
