...
export class SyncSchemaWithEntities1783077482112
  implements MigrationInterface
{
  name = 'SyncSchemaWithEntities1783077482112';

  public async up(queryRunner: QueryRunner): Promise<void> {
    // 1. roles: add is_system_role column
    await queryRunner.query(`
      ALTER TABLE "roles"
      ADD COLUMN IF NOT EXISTS "is_system_role" boolean NOT NULL DEFAULT false
    `);

    // 2. projects: add extended_metadata column
    await queryRunner.query(`
      ALTER TABLE "projects"
      ADD COLUMN IF NOT EXISTS "extended_metadata" jsonb
    `);

    // 3. user_roles: add id SERIAL, created_at, updated_at;
    //    drop composite PK so department_id can be nullable, then add PK on id
    await queryRunner.query(`
    ...
    // Re-add uniqueness on (user_id, department_id, role_id) that was enforced by composite PK
    await queryRunner.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
    ...
    // 4. users: make department_id nullable (entity has nullable: true)
    await queryRunner.query(`
      ALTER TABLE "users"
      ALTER COLUMN "department_id" DROP NOT NULL
    `);

    // 5. approval_steps: make department_id and level_rank nullable
    await queryRunner.query(`
    ...
    // 10. Create permission_snapshot_history table
    await queryRunner.query(`
    ...
    // 11. Create user_project_feature_matrix table
    await queryRunner.query(`
